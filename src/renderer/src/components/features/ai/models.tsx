...
export default function Models({
	...
	setOllamaSupport: (support: any) => void;
	ollamaModel: string;
}) {
	...
	const handleDownloadModel = async (model: string) => {
		...
		const response = await apiFetch(
			`/ai/ollama/download-model?model=${model}`,
			{
				method: "POST",
			},
		);
	...
	return (
		<section className="w-full h-full flex flex-col items-center justify-start pt-16">
			<div
				id="logs"
				className="w-full h-full max-h-[calc(100vh-23rem)] overflow-y-auto"
			>
				<div className="w-full max-w-2xl mx-auto">
					...
					{Object.entries(availableModels).map(([groupKey, group]) => (
						<div key={groupKey} className="mb-6 w-full">
							...
							<p className="text-sm text-neutral-400 mb-3 text-pretty">
								{group.description}
							</p>
									...
									(model: { id: string; name: string; size: string }) => {
										...
										return (
											...
											>
												...
												<p className="text-[11px] text-neutral-400 uppercase z-10">
													{model.size}
												</p>
											</div>
										);
