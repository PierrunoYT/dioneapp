...
class BuildToolsManager {
	...
	private resolveManagedBuildTools(): BuildToolsVerification | null {
		...
		logger.info(
			`Using Visual Studio Build Tools located at ${verification.installPath}`,
		);
		return verification;
	}
...
echo VCVARS_ENV_END`;
	...
	async initialize(): Promise<boolean> {
		...
		if (process.platform !== "win32") {
			logger.info(
				"Build tools initialization skipped on non-Windows platform.",
			);
			return false;
		}
		...
		try {
			...
			if (!vcvarsEnv) {
				logger.error(
					"Failed to compute environment variables from vcvars64.bat",
				);
				return false;
			}
	...
	getEnhancedEnvironment(
		...
		if (baseEnv.PATH) {
			const separator = ";";
			const vcPath = this.vcvarsEnv.PATH
				? this.vcvarsEnv.PATH.split(separator)
				: [];
